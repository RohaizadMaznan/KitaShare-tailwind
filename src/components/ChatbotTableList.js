...
export default function UploadTable({
  ...
  const fileAnonymous = () => {
    ...
    db.collection("chatbots")
      ...
      .then(() => {
        console.log(`Upload is hiding ${chatbotId}`);
        const message = `${number} is now hiding anonymously.`;
        addToast(message, {
          appearance: "success",
          autoDismiss: true,
        });
  ...
  const fileShow = () => {
    ...
    db.collection("chatbots")
      ...
      .then(() => {
        console.log(`Chatbot is hiding ${chatbotId}`);
        const message = `${number} is now showing to public.`;
        addToast(message, {
          appearance: "success",
          autoDismiss: true,
        });
        ...
        <td className="px-4 py-4 text-center flex justify-center">
          ...
          {fileShowHide === true ? (
            <>
              <span
                onClick={fileShow}
                className="hover:text-blue-600 hover:underline cursor-pointer mx-1"
                title="Show"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
